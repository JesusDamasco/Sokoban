//nasm -f win64 Assembly.asm -o Map.o
//gcc Generar.c Map.o -o Generar.exe
//./Generar.exe
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
char **generaTablero(int n, int *jugadorX, int *jugadorY, int *cajasRestantes, int posObjetivos[4][2]);
void imprimeTablero(char **tablero, int n, int cajas, Mapa *mapa);
...
int main() {
    ...
    mapa.columnas = tam;
    int posObjetivos[4][2]; 
    mapa.tablero = generaTablero(tam, &mapa.jugadorX, &mapa.jugadorY, &mapa.cajasRestantes, posObjetivos);
    memcpy(mapa.posObjetivos, posObjetivos, sizeof(posObjetivos));
    int box;
    char tecla;
    int dx, dy;
    
    system("cls");
    //printf("Usa WASD para moverte, y Q para salir\n");
    //printf("Cajas restantes: %d\n", mapa.cajasRestantes);

    do {
        imprimeTablero(mapa.tablero, tam, mapa.cajasRestantes, &mapa);
        box = mapa.cajasRestantes;
...
void imprimeTablero(char **tablero, int n, int cajas, Mapa *mapa) {
    ...
    //Restauracion de la X
    for (int i = 0; i < 3; i++) {
        int y = mapa->posObjetivos[i][0];
        int x = mapa->posObjetivos[i][1];
        if (tablero[y][x] == ' ') {
            tablero[y][x] = 'X';
        }
    }
    ...
    //Borrado de la X
    if (tablero[mapa->jugadorY][mapa->jugadorX] == 'X') {
        tablero[mapa->jugadorY][mapa->jugadorX] = 'P';
    }
...
void Victoria() {
    ...
    printf("    *                                   *\n");
    printf("    *             GANASTE!!             *\n");
    printf("    * Todas las cajas estan en su lugar *\n");
    printf("    *                                   *\n");
    printf("    *************************************\n");
